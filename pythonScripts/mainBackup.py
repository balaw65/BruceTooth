...
# TODO I think I can move these classes to another file:
...
#        print pDevice
   ...
   def pairDeviceThread(self,s):
      print ("PAIRING THREAD:  Attempting to pair device at address: %s" % s)
      # addressToPair = s
      devices = Devices()
      devices.pairDevice(s)

   def PairDevice(self,s):
      print("Pair Deviced Called!!!!!!!!!!!!!!!!!!!!!%s" % s)
      t = Thread(target=self.pairDeviceThread,args=(s,))
      t.start()
      print("Pair Device Thread activated")
...
#def PairingThread():
#  while not QUIT: 
#     #if len(ADDRESSTOPAIR) > 0:
#     print ("ATTEMPT TO PAIR: |%s|" % ADDRESSTOPAIR)
#     time.sleep(1)
...
# ------------   AGENT:        --------------
   ...
   agent = Agent(systemBus, path)

   mainloop = GObject.MainLoop()

   obj = systemBus.get_object(BUS_NAME, "/org/bluez");
